...
namespace pyro {
    ...
    void PyroRender::draw_frame() {
        vkWaitForFences(device.get_logical_device(), 1, &device.get_inflight_fence()[current_frame], VK_TRUE, UINT64_MAX);
        ...
        ASSERT_EQUAL(vkAcquireNextImageKHR(device.get_logical_device(), device.get_swap_chain(), UINT64_MAX,
                                           device.get_image_available_semaphore()[current_frame], VK_NULL_HANDLE, &image_index),
                     VK_SUCCESS, "Failed to Acquire next image")
        vkResetCommandBuffer(device.get_command_buffer()[current_frame], 0);
        VulkanDevice::record_command_buffer(device.get_command_buffer()[current_frame], image_index, pyroPipeline.get_render_pass(),
        ...
        ASSERT_EQUAL(vkQueueSubmit(device.get_graphics_queue(), 1, &submit_info, device.get_inflight_fence()[current_frame]),
